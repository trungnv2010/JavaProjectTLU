...
const PRESETS: Preset[] = [
  { name: 'today', label: t('calendar.today') },
  { name: 'yesterday', label: t('calendar.yesterday') },
  { name: 'last7', label: t('calendar.last7Days') },
  { name: 'last14', label: t('calendar.last14Days') },
  { name: 'last30', label: t('calendar.last30Days') },
  { name: 'thisWeek', label: t('calendar.thisWeek') },
  { name: 'lastWeek', label: t('calendar.lastWeek') },
  { name: 'thisMonth', label: t('calendar.thisMonth') },
  { name: 'lastMonth', label: t('calendar.lastMonth') }
]
...
export const DateRangePicker: FC<DateRangePickerProps> & {
  ...
  const PresetButton = ({
    ...
    <Button
      className={cn(isSelected && 'pointer-events-none bg-primary font-semibold text-muted')}
      variant='ghost'
      onClick={() => {
        setPreset(preset)
      }}
      ...
      <PopoverTrigger asChild>
        <Button className={className} size={'lg'} variant='outline'>
          ...
          <div className='-mr-2 pl-1 opacity-60'>
            {isOpen ? <CalendarDays size={24} /> : <CalendarDays size={24} />}
          </div>
        </Button>
      </PopoverTrigger>
                ...
                {showCompare && (
                  <div className='flex items-center space-x-2 py-1 pr-4'>
                    <Switch
                      ...
                      id='compare-mode'
                    />
                    <Label htmlFor='compare-mode'>{t('button.compare')}</Label>
                  </div>
                )}
